...
- [x] `workflows/ci.yml` (push `main`/`feat/**` + PR→main): pnpm@9 + `actions/setup-node@v4 cache:pnpm` → **biome CHỈ file đổi** (baseline repo KHÔNG biome-clean: 326 lỗi phần lớn CRLF-local → gate repo-wide sẽ fail giả) → `turbo typecheck` → `turbo build` → `turbo test` (**`--concurrency=2` từ 2026-07-31** + `timeout-minutes: 30` cho job `verify` — lý do & số đo ở `product-roadmap.md` §8). Job `changeset` (PR-only) chạy `changeset status --since=origin/<base>`. `fetch-depth:0` để có base cho diff + changeset. `concurrency` cancel-in-progress.
